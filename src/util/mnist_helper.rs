...
use crate::train::layer_container::{LayerContainer, cross_entropy_loss_derivative_and_softmax};
...
pub fn load_mnist_without_label(path: &str) -> Result<Array2<f32>, Box<dyn Error>> {
    ...
    for line in reader.lines() {
        ...
        let features: Vec<f32> = parts[..].iter().filter_map(|s| s.parse().ok()).collect();
        X.push(features);
...
pub fn load_mnist(path: &str) -> Result<(Array2<f32>, Array1<f32>), Box<dyn Error>> {
    ...
    for line in reader.lines() {
        ...
        let parts: Vec<&str> = line.split(',').collect();
        let class = parts[0];
        let features: Vec<f32> = parts[1..].iter().filter_map(|s| s.parse().ok()).collect();
        X.push(features);
...
pub fn train_mnist_cnn(x: Array2<f32>, y: Array1<f32>) -> Option<f32> {
    ...
    let layers: Vec<Box<dyn crate::train::layerable::Layerable>> =
        vec![Box::new(aaa), Box::new(ActivationLayer::relu())];
